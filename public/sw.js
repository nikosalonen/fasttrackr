...
self.addEventListener("install", (event) => {
	...
	event.waitUntil(
		caches
			...
			.then(() => {
				console.log(
					"Service Worker: All files cached, forcing immediate activation",
				);
...
self.addEventListener("fetch", (event) => {
	...
	if (
		NETWORK_FIRST_URLS.some(
			(path) => url.pathname === path || url.pathname.startsWith("/src/"),
		)
	) {
...
self.addEventListener("push", (event) => {
	...
	event.waitUntil(self.registration.showNotification("FastTrackr", options));
});
